...
| `@import "stl-design-system/tokens"` | 디자인 토큰 + 전역 스타일 로드   | 커스텀 색상/폰트 미적용 |
| `@source "..."`                      | 디자인 시스템 클래스 스캔        | 컴포넌트 스타일 미적용  |

### ⚠️ `stl-design-system/styles` 는 임포트하지 마세요

패키지에 CSS 진입점이 두 개 있지만, **사용처가 임포트하는 건 `tokens` 하나뿐**입니다.

| 진입점                       | 대상                    | 설명                                                                                  |
| ---------------------------- | ----------------------- | ------------------------------------------------------------------------------------- |
| `stl-design-system/tokens`   | **사용처 (이걸 쓰세요)** | 디자인 토큰 + 전역 스타일. 폰트를 base64 로 인라인해 node_modules 경로 문제가 없습니다 |
| `stl-design-system/styles`   | DS 내부 / Storybook 전용 | 자체적으로 `@import "tailwindcss"` 를 하므로 사용처가 임포트하면 **tailwind 중복 로드** |

전역 스타일(스크롤바 등)은 두 파일에 중복 정의되어 있어 `tokens` 만으로 충분합니다.

> DS 를 수정하는 경우: 전역 스타일을 바꿀 때 **`globals.css` 와 `tokens.css` 를 반드시 함께** 고쳐야 합니다. 한쪽만 고치면 Storybook 에서는 반영되는데 사용처에서는 안 먹습니다.
