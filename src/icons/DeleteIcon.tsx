import type { IconProps } from './types'

export function DeleteIcon({ size = 24, className, ...props }: IconProps) {
  // 24 이하: 24x24 아이콘 사용, 24 초과: 32x32 아이콘 사용
  const useSmall = size <= 24

  if (useSmall) {
    return (
      <svg
        width={size}
        height={size}
        viewBox="0 0 24 24"
        fill="none"
        xmlns="http://www.w3.org/2000/svg"
        className={className}
        {...props}
      >
        <path d="M7.7002 18.7002H16.2998V9.60059H17.2998V19.2002L16.7998 19.7002H7.2002L6.7002 19.2002V9.60059H7.7002V18.7002ZM10.0996 16.7998H9.09961V9.60059H10.0996V16.7998ZM12.5 16.7998H11.5V9.60059H12.5V16.7998ZM14.9004 16.7998H13.9004V9.60059H14.9004V16.7998ZM18.5 7.2002V9.60059H17.5V7.7002H6.5V9.60059H5.5V7.2002L6 6.7002H18L18.5 7.2002ZM14.4004 4.2998V5.2998H9.59961V4.2998H14.4004Z" fill="currentColor"/>
      </svg>
    )
  }

  return (
    <svg
      width={size}
      height={size}
      viewBox="0 0 32 32"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
      className={className}
      {...props}
    >
      <path d="M10.1002 25.0979H21.901V12.8H22.901V25.5979L22.401 26.0979H9.60022L9.10022 25.5979V12.8H10.1002V25.0979ZM13.3004 22.3997H12.3004V12.8H13.3004V22.3997ZM16.4987 22.3997H15.4987V12.8H16.4987V22.3997ZM19.6998 22.3997H18.6998V12.8H19.6998V22.3997ZM24.4987 9.59985V12.801H23.4987V10.0999H8.50061V12.801H7.50061V9.59985L8.00061 9.09985H23.9987L24.4987 9.59985ZM19.1989 6.89868H12.8004V5.89868H19.1989V6.89868Z" fill="currentColor"/>
    </svg>
  )
}
